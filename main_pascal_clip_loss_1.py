...
def cluster_contrastive(img_dim,text_dim):
    ...
    model=CCMR(img_dim,text_dim,n_clusters=args.n_clusters,n_h=args.n_h,alpha=1.0,pretrain_path='result300/pascal/_64_119_UCMR.pt').to(device)
    print(model)
    #besti2_t_list,bestt2_i_list,best_map_list=model.pretrain_MLP()
    criterion1 = lambda x, y: (((x - y) ** 2).sum(1).sqrt()).mean().to(device)

    model.pretrain_MLP('result300/pascal/_64_119_UCMR.pt')
...
if __name__ == '__main__':
    ...
    from dataloader1 import load_data,MultiviewDataset,RandomSampler,FullDataset
