...
def cluster_contrastive(img_dim,text_dim):
    ...
    model=CCMR(img_dim,text_dim,n_clusters=args.n_clusters,n_h=args.n_h,alpha=1.0,pretrain_path='result300/wikipedia/_128_85_UCMR.pt').to(device)
    print(model)
...
if __name__ == '__main__':
    ...
    for i in range(args.T + 1):
        ...
        best_map, besti2_t, bestt2_i = cluster_contrastive(dims[0], dims[1])
        result_record["map"].append(best_map)
        result_record["i2t"].append(besti2_t)
        result_record["t2i"].append(bestt2_i)
    print("----------------Training Finish----------------")
    print("----------------Final Results----------------")
    print("Average Map (mean) = {:.4f} map (std) = {:.4f}".format(np.mean(result_record["map"]),
                                                                  np.std(result_record["map"])))
    print("Average i2t (mean) = {:.4f} Average i2t (std) = {:.4f}".format(np.mean(result_record["i2t"]),
                                                                          np.std(result_record["i2t"])))
    print("Average t2i (mean) = {:.4f} Average t2i (std) = {:.4f}".format(np.mean(result_record["t2i"]),
                                                                          np.std(result_record["t2i"])))
