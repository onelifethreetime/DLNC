...
if __name__ == '__main__':
    ...
    b=np.where(mask[:,0]==1)[0] #only image index add complete data
    #d=np.setdiff1d(b,a) #image index
    #c=mask[b]
    mv_data = []
    ...
    sum_vec1=np.sum(mask[b],axis=1,keepdims=True)
    mask=mask[b]
    incomplete_ind = (sum_vec1[:, 0]) != 2
