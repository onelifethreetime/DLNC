...
from loss_pas import Contrastive
...
def evaluate1(model):
    ...
    print('Eval loss: ',running_loss/len(loader),'c_loss:',c_total / len(loader),'mse_loss:',mse_total / len(loader))
...
def evaluate(model):
    ...
    with torch.no_grad():
        for  xs,label,idx in loader:
            ...
            contrastive_loss=contrastive(model.ae,cat_feature,ypred)
...
def make_imputation(mask, indices, incomplete_ind,K1):
    global data_list
    K=K1
    ...
    def update_feature(self, model, loader, mask, incomplete_ind, epoch):
        ...
        if epoch == 1:
            ...
            for v in range(2):
                ...
                indices.append(ind[:, 1:])
            if self.bi:
                make_imputation(mask, indices, incomplete_ind,K)
            return indices
...
def figure_plt(Train_Loss, Valid_Loss):
    ...
    plt.ylabel('Loss')
    plt.show()
    plt.savefig('result800/{}_loss.pdf'.format(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))))
def figure_plt_map_epoch(I2T,T2I,mAP,dataset_name):
    plt.figure()
    Epoch=len(mAP)
    X=range(1,Epoch+1)
    plt.plot(X, I2T, label='Image2TextmAP ')
    plt.plot(X, T2I, label='Text2ImagemAP ')
    plt.plot(X,mAP,label='Average mAP ')
    plt.legend()
    plt.xlabel('Epoch')
    plt.ylabel('mAP')
    plt.savefig('result800/{}_map.pdf'.format(dataset_name))
    plt.show()
...
def pretrain(model):
    ...
    for epoch in range(150):
        ...
        print('Pretrain_epoch {}'.format(epoch), 'loss:{:.6f}'.format(tot_loss / len(data_loader)),'c_loss:{:.6f}'.format(c_total / len(data_loader)),'mse_loss:{:.6f}'.format(mse_total/ len(data_loader)))
        ...
        if (img2text + text2img) / 2. > best_map:
            ...
            besti2_t_list.append(besti2_t)
            bestt2_i_list.append(bestt2_i)
    ...
    return besti2_t_list,bestt2_i_list,best_map_list



def cluster_contrastive(img_dim,text_dim,K):
    ...
    print(model)
    #besti2_t_list,bestt2_i_list,best_map_list=model.pretrain_MLP()
    criterion1 = lambda x, y: (((x - y) ** 2).sum(1).sqrt()).mean().to(device)

    model.pretrain_MLP('result300/pascal/_128_122_UCMR.pt')
    ...
    for epoch in range(150):
        ...
        mse_loss = criterion1(z1, z2)
        #loss = args.alpha * c_loss + mse_loss

        loss = kl_loss + args.alpha *contrastive_loss+args.beta*mse_loss
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        img2text, text2img, t_imgs, t_txts, t_labels,eval_loss=evaluate(model)
        ...
        test_txt_acc_history.append(text2img)
        if (img2text + text2img) / 2. > best_map:
            best_map = (img2text + text2img) / 2.
            besti2_t = img2text
            bestt2_i = text2img
    ...
    #figure_plt(train_loss_history,test_loss_history)
    #figure_plt_map_epoch(besti2_t_list,bestt2_i_list,best_map_list, args.dataset)
    return best_map,besti2_t,bestt2_i
...
if __name__ == '__main__':
    ...
    parser.add_argument("--beta", default=0.01)#[10,1,0.1,0.01]
    parser.add_argument("--alpha", default=100) #[10,1,0.1,0.01,0.001]
    ...
    parser.add_argument('--miss_rate', type=int, default=5, help='rate of miss')
    ...
    result_record = {"k":[],"map": [], "i2t": [], "t2i": []}
    random_seed=[1,10,100,1000,1001,1002,1003,2000,2001,2002,2003]#,10,100,1000,1001,1002,1003,2000,2001,2002,2003
    for seed in random_seed:
        ...
        for k in range(20):
            for i in range(args.T+1):
                ...
                best_map,besti2_t,bestt2_i=cluster_contrastive(dims[0],dims[1],K)

                result_record["k"].append(K)
                result_record["map"].append(best_map)
                result_record["i2t"].append(besti2_t)
                result_record["t2i"].append(bestt2_i)
    print("----------------Training Finish----------------")
    print("----------------Final Results----------------")
    print('Best K')
    print(result_record["k"])
    print('Best Map')
    print(result_record["map"])
    print('Best i2t')
    print(result_record["i2t"])
    print('Best t2i')
    print(result_record["t2i"])

    print("Average Map (mean) = {:.4f} map (std) = {:.4f}".format(np.mean(result_record["map"]), np.std(result_record["map"])))
    print("Average i2t (mean) = {:.4f} Average i2t (std) = {:.4f}".format(np.mean(result_record["i2t"]), np.std(result_record["i2t"])))
    print("Average t2i (mean) = {:.4f} Average t2i (std) = {:.4f}".format(np.mean(result_record["t2i"]), np.std(result_record["t2i"])))
