...
def pretrain(model):
    ...
    for epoch in range(150):
        ...
        for batch_idx,(xs,_,_) in enumerate(data_loader):
            ...
            c_loss=criterion.forward_feature(z0,z1)
            mse_loss=criterion1(z0,z1)
            loss=args.alpha*c_loss+args.beta*mse_loss
            loss.backward()
            optimizer.step()
            tot_loss+=loss.item()
            c_total+=c_loss.item()
...
def cluster_contrastive(img_dim,text_dim):
    ...
    model = CCMR(img_dim, text_dim, n_clusters=args.n_clusters, n_h=args.n_h, alpha=1.0,
                 pretrain_path='result300/wikipedia/_128_85_UCMR.pt').to(device)
    print(model)
    # besti2_t_list,bestt2_i_list,best_map_list=model.pretrain_MLP()
    criterion1 = lambda x, y: (((x - y) ** 2).sum(1).sqrt()).mean().to(device)

    model.pretrain_MLP('result300/wikipedia/_128_85_UCMR.pt')
    ...
    for epoch in range(150):
        ...
        if acc > best_acc2:
            ...
            best_f12 = np.copy(f1)
            best_epoch = epoch
        print('best epoch {}'.format(epoch), ':best_acc {:.4f}'.format(acc), ',best_nmi {:.4f}'.format(nmi),
              ',best_ari {:.4f}'.format(ari), ':Acc {:.4f}'.format(acc),
        ...
        tot_loss += loss.item()
        print('epoch {}'.format(epoch), 'contrastive Loss:{:.6f}'.format(tot_con), 'kl Loss:{:.6f}'.format(tot_kl),
              'Loss:{:.6f}'.format(tot_loss))
        ...
        if (img2text + text2img) / 2. > best_map:
            ...
            best_model_wts = copy.deepcopy(model.state_dict())
            dir = os.getcwd()
            dirs_path = os.path.join(dir, 'result600/{}/{}_{}_{}'.format(args.dataset, args.dataset, args.batch_size,
                                                                         args.beta))
            if not os.path.exists(dirs_path):
                os.makedirs(dirs_path)
            torch.save(model.state_dict(),
                       'result600/{}/{}_{}_{}/{}_UCMR.pt'.format(args.dataset, args.dataset, args.batch_size, args.beta,
                                                                 epoch))
            np.savez(
                'result600/{}/{}_{}_{}/{}_{}.npz'.format(args.dataset, args.dataset, args.batch_size, args.beta, epoch,
                                                         best_map), image=t_imgs, text=t_txts, label=t_labels)
            no_up = 0
        else:
            # lr_schedu.step(best_map)
            no_up += 1
...
if __name__ == '__main__':
    ...
    Dataname = 'wikipedia' #wikipedia,nus_wide,pascal,xmedianet
    parser = argparse.ArgumentParser(description='train')
    parser.add_argument('--dataset', default=Dataname)
    parser.add_argument('--batch_size', default=128, type=int)
    parser.add_argument("--temperature_f", default=0.5)
    parser.add_argument("--beta", default=0.001)#[10,1,0.1,0.01]
    parser.add_argument("--alpha", default=10) #[10,1,0.1,0.01,0.001]
