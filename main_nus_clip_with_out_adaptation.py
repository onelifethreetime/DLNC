...
from loss_n_adaptation import Contrastive
...
def pretrain(model):
    ...
    for epoch in range(150):
        ...
        if (img2text + text2img) / 2. > best_map:
            ...
            dir = os.getcwd()
            dirs_path = os.path.join(dir,'result500/{}'.format(args.dataset))
            if not os.path.exists(dirs_path):
                os.makedirs(dirs_path)
            torch.save(model.state_dict(),'result500/{}/_{}_{}_UCMR.pt'.format(args.dataset,args.batch_size, epoch))
            np.savez('result500/{}/_{}_{}_{}.npz'.format(args.dataset,args.batch_size, epoch, best_map),image=t_imgs, text=t_txts, label=t_labels)
            no_up = 0
        else:
            no_up += 1
...
def cluster_contrastive(img_dim,text_dim):
    ...
    model=CCMR(img_dim,text_dim,n_clusters=args.n_clusters,n_h=args.n_h,alpha=1.0,pretrain_path='result500/pascal/_128_29_UCMR.pt').to(device)
    print(model)
    #model.pretrain_MLP()

    model.pretrain_MLP('result500/nus_wide/_128_29_UCMR.pt')
    ...
    for epoch in range(150):
        ...
        if (img2text + text2img) / 2. > best_map:
            ...
            best_model_wts = copy.deepcopy(model.state_dict())
            dir = os.getcwd()
            dirs_path = os.path.join(dir,'result500/{}/{}_{}_{}'.format(args.dataset, args.dataset,args.batch_size,args.beta))
            if not os.path.exists(dirs_path):
                os.makedirs(dirs_path)
            torch.save(model.state_dict(),'result500/{}/{}_{}_{}/{}_UCMR.pt'.format(args.dataset, args.dataset,args.batch_size,args.beta, epoch))
            np.savez('result500/{}/{}_{}_{}/{}_{}.npz'.format(args.dataset, args.dataset,args.batch_size,args.beta, epoch, best_map),image=t_imgs, text=t_txts, label=t_labels)
            no_up = 0
        else:
            # lr_schedu.step(best_map)
            no_up += 1
