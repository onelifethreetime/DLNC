...
def extract_features(model, data_list, print_freq=50,flip=True,mode=0):
    ...
    with torch.no_grad():
        ...
        img_feature,txt_feature=model(data1,data2)
        img_feature= F.normalize(data1,dim=1)
        txt_feature=F.normalize(data2,dim=1)
...
def cluster_contrastive(img_dim,text_dim):
    ...
    model=Model_3(img_dim,text_dim).to(device)
    print(model)
    pretrain(model)


    path='result300/nus_wide/_128_29_UCMR.pt'
    model.load_state_dict(torch.load(path))
    print('load pretrained ae from', path)

    optimizer = torch.optim.Adam(model.parameters(), lr=args.learning_rate)
    lr_schedu = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=10, threshold=0.0001,
                                                     threshold_mode='rel', cooldown=0, min_lr=0, eps=1e-08,
                                                     verbose=False)

    from trainers import ClusterContrastTrainer
    ...
    for epoch in range(args.epochs):
        with torch.no_grad():
            if epoch == 0:
                i_eps = 0.50
                print('Image Clustering criterion: eps: {:.3f}'.format(i_eps))
                cluster_image = DBSCAN(eps=i_eps, min_samples=10, metric='precomputed', n_jobs=-1)
                t_eps = 0.50
                print('text Clustering criterion: eps: {:.3f}'.format(t_eps))
                cluster_text = DBSCAN(eps=t_eps, min_samples=10, metric='precomputed', n_jobs=-1)
        print('==> Create pseudo labels for unlabeled image-text data')

        feature_image, feature_text = extract_features(model, data_list)

        from utils.faiss_rerank import compute_jaccard_distance
        rerank_dist_image = compute_jaccard_distance(feature_image, k1=args.k1, k2=args.k2,
                                                     search_option=3)  # rerank_dist_all_jacard[features_rgb.size(0):,features_rgb.size(0):]#
        pseudo_labels_image = cluster_image.fit_predict(rerank_dist_image)

        rerank_dist_text = compute_jaccard_distance(feature_text, k1=args.k1, k2=args.k2,
                                                    search_option=3)  # rerank_dist_all_jacard[:features_rgb.size(0),:features_rgb.size(0)]#
        pseudo_labels_text = cluster_text.fit_predict(rerank_dist_text)

        num_cluster_image = len(set(pseudo_labels_image)) - (1 if -1 in pseudo_labels_image else 0)
        num_cluster_text = len(set(pseudo_labels_text)) - (1 if -1 in pseudo_labels_text else 0)

        print('==> Statistics for Image epoch {}: {} clusters'.format(epoch, num_cluster_image))
        print('==> Statistics for text epoch {}: {} clusters'.format(epoch, num_cluster_text))
        print('the nunmber of pseudo_labels_image -1 is {}'.format(len(np.where(pseudo_labels_image == -1)[0])))
        print('the nunmber of pseudo_labels_text -1 is {}'.format(len(np.where(pseudo_labels_text == -1)[0])))  #
        ...
        cluster_features_image = generate_cluster_features(pseudo_labels_image, feature_image)
        cluster_features_text = generate_cluster_features(pseudo_labels_text, feature_text)
